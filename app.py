...
IS_DEPLOYED = (
    os.environ.get("SPACE_ID") is not None or os.environ.get("VERCEL") is not None
)
...
MODEL_PATH = PROJECT_ROOT / "models" / "unified_best_model.pkl"  # calibrated
RAW_MODEL_PATH = PROJECT_ROOT / "models" / "unified_raw_model.pkl"  # for SHAP
PREPROCESSOR_PATH = PROJECT_ROOT / "models" / "unified_preprocessor.pkl"
SEGMENT_BINS_PATH = PROJECT_ROOT / "models" / "unified_segment_bins.pkl"
...
def render_shap_deep_dive(raw_model, X_processed, feature_names):
    """
...
/* Hide blue page icon that appears next to every heading */
[data-testid="stPageSectionHeader"] {{ display: none !important; }}
.stApp [data-baseweb="block"] img {{ display: none !important; }}
[data-testid="stTitle"] img,
[data-testid="stHeadingWithActionElements"] img {{ display: none !important; }}
button[data-testid="stPageLink"] {{ display: none !important; }}
...
def load_model_cached():
    ...
    raw_model = joblib.load(RAW_MODEL_PATH) if RAW_MODEL_PATH.exists() else model
    preprocessor = joblib.load(PREPROCESSOR_PATH)
    seg_bins = joblib.load(SEGMENT_BINS_PATH) if SEGMENT_BINS_PATH.exists() else None
    return model, raw_model, preprocessor, seg_bins


with st.spinner("⏳ Loading model..."):
    model, raw_model, preprocessor, segment_bins = load_model_cached()

st.sidebar.markdown(
    """
<div style='text-align:center; padding:16px 0 8px;'>
    <div style='font-size:22px; font-weight:700; color:#818cf8;'>📊 Churn Predictor</div>
    <div style='font-size:12px; color:#555; margin-top:4px;'>Cross-Industry Platform</div>
</div>
""",
    unsafe_allow_html=True,
)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("🚀 Predict Churn", type="primary"):
        try:
            ...
            # ── Encode string columns before feature engineering ───────────
            ...
            # source_industry: keep as string (handled by OneHotEncoder)
            df["source_industry"] = (
                df["source_industry"].astype(str).str.lower().str.strip()
            )

            # is_active: yes/no/true/false → 1/0
            ...
            df = create_engineered_features(df, segment_bins=segment_bins)
        ...
        try:
            ...
            if base_vals:
                ...
                input_df = create_engineered_features(
                    input_df, segment_bins=segment_bins
                )
                for col in ["value_segment", "tenure_segment", "engagement_segment"]:
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        st.caption(
            "Manually trigger a retraining run. This runs your full training pipeline and registers the new model."
        )
        ...
        with r_col2:
            if st.button(
                "🚀 Trigger retraining", type="primary", use_container_width=True
            ):
                import subprocess, tempfile, os, time

                st.info("⏳ Retraining started — log updates every 3 seconds...")
                log_box = st.empty()
                status_box = st.empty()

                # Write output to a temp file so we can stream it live
                log_file = PROJECT_ROOT / "retraining.log"
                ...
                try:
                    ...
                    # Stream log live while process runs
                    ...
                    while proc.poll() is None and elapsed < max_wait:
                        ...
                        try:
                            ...
                            # Show last 60 lines only (avoid huge text box)
                            lines = log_content.strip().splitlines()
                            shown = "\n".join(lines[-60:])
                            log_box.code(shown, language="text")
                            status_box.caption(f"⏱️ Running... {elapsed}s elapsed")
                        except Exception:
                            pass
                    ...
                    if return_code == 0:
                        status_box.success(
                            "✅ Retraining complete! Refresh the page to see updated runs."
                        )
                        last_lines = "\n".join(final_log.strip().splitlines()[-80:])
                        log_box.code(last_lines, language="text")
                        # Clear cache so new model is loaded
                        st.cache_resource.clear()
                    elif return_code is None:
                        status_box.warning(
                            "⚠️ Retraining timed out (10 min). Check retraining.log in your project folder."
                        )
                    else:
                        status_box.error(
                            f"❌ Retraining failed (exit code {return_code}). See log below."
                        )
                        log_box.code(final_log[-3000:], language="text")

                except Exception as train_err:
                    status_box.error(f"Could not start retraining: {train_err}")
