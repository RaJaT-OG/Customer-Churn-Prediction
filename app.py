...
# HF Spaces automatically sets SPACE_ID — use it to detect cloud deployment
IS_DEPLOYED = os.environ.get("SPACE_ID") is not None
...
def _compatible_columns(target: str, user_df: pd.DataFrame) -> list[str]:
    ...
    for col in user_df.columns:
        ...
        if expected == "numeric" and is_num:
            compatible.append(col)
        elif expected == "binary" and (is_num or n_uniq <= 5):
            compatible.append(col)
        elif expected == "categorical" and (is_num or n_uniq <= 25):
            compatible.append(col)
...
def _confidence_badge(score: float, warning: str | None = None) -> tuple[str, str]:
    ...
    if score >= 0.85:
        return "Auto-matched ✓", "#27ae60"
    if score >= 0.60:
        return "Good match", "#2980b9"
    if score >= 0.45:
        return "Possible match", "#e67e22"
...
def render_smart_mapper(user_df: pd.DataFrame) -> dict | None:
    ...
    for target, (col, score) in sorted(raw_scores.items(), key=lambda x: -x[1][1]):
        if col is None:
    ...
    needs_review = sum(
        1
        for t, col in auto_matches.items()
        if col is not None
        and (raw_scores[t][1] < 0.85 or _value_range_check(t, col, user_df) is not None)
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Total fields", total)
    c2.metric(
        "Auto-matched", matched, delta=f"{matched/total:.0%} done", delta_color="normal"
    )
    c3.metric(
        "Needs review", needs_review, delta_color="inverse" if needs_review else "off"
    )
    ...
    for i, target in enumerate(BASE_COLUMNS):
        ...
        value_warning = (
            _value_range_check(target, auto_col, user_df) if auto_col else None
        )
        ...
        with map_cols[i % 2]:
            ...
            if chosen == "— none —":
                ...
                live_score = max(
                    ...
                    max(
                        (_fuzzy_score(s, chosen) for s in schema["synonyms"]),
                        default=0.0,
                    ),
                )

            live_badge_label, live_badge_color = _confidence_badge(
                live_score, live_warning
            )
            ...
            if live_warning:
                ...
                if chosen != "— none —" and pd.api.types.is_numeric_dtype(
                    user_df[chosen]
                ):
                    if st.checkbox(
                        ...
                        st.success(
                            f"✅ `{chosen}` normalised (divided by {col_max:.2f})"
                        )
                ...
                [
                    {
                        ...
                        "Type OK": (
                            "✅"
                            if c in _compatible_columns(t, user_df)
                            else "⚠️ Review"
                        ),
                    }
    ...
    if not null_cols.empty:
        for col, count in null_cols.items():
            ...
            if pct > 30:
                ...
                warnings.append(
                    msg + " Missing values will be filled with column median/mode."
                )
...
def _run_shap(explainer, X_dense):
    ...
    Handles every output format:
      - list of arrays [class0, class1, ...] → older SHAP convention
      - single 2D array (n_samples, n_features) → XGBoost binary / single-output
      - single 3D array → RandomForest-style multiclass output. SHAP's axis
        order for 3D arrays differs across versions:
          old:  (n_classes, n_samples, n_features)
          new:  (n_samples, n_features, n_classes)
        Rather than assume one layout (which breaks — e.g. indexing axis 0
        at position 1 fails whenever n_samples == 1, such as a single-
        customer lookup), we detect which axis actually matches the number
        of rows we passed in and select the positive/churn class from
        whichever axis holds the classes.
    """
    import numpy as np
    ...
    # Case 1: list of per-class arrays, e.g. [class0, class1]
    if isinstance(vals, list):
        # Positive/churn class is conventionally the last entry
        return np.array(vals[-1])

    arr = np.array(vals)

    # Case 2: already 2D — XGBoost binary or single-output models
    if arr.ndim == 2:
        return arr

    # Case 3: 3D — figure out which axis holds the samples vs. the classes
    if arr.ndim == 3:
        if arr.shape[0] == n_samples:
            # (n_samples, n_features, n_classes) — take last class
            return arr[:, :, -1]
        elif arr.shape[1] == n_samples:
            # (n_classes, n_samples, n_features) — take last class
            return arr[-1]
        else:
            # Unrecognised layout — fall back to assuming the last axis
            # is the class dimension.
            return arr[..., -1]
...
def compute_shap_reasons(model, X_processed, feature_names, top_n=3, max_rows=500):
    """
    ...
    try:
        ...
        if n_total > max_rows:
            ...
            shap_full = np.vstack(
                [shap_sample, np.tile(mean_vals, (n_total - max_rows, 1))]
            )
        else:
...
def render_shap_deep_dive(model, X_processed, feature_names):
    ...
    st.caption(
        "SHAP values show which features push customers toward or away from churning."
    )
...
if st.session_state["theme"] == "dark":
    BG = "linear-gradient(135deg, #050505 0%, #0d0d0d 100%)"
    CARD_BG = "rgba(24, 24, 24, 0.85)"
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander(
        "✅ Column mapping confirmed — click to review or change", expanded=False
    ):
        summary = pd.DataFrame(
            [{"Model field": k, "→ Your column": v} for k, v in mapping.items()]
        )
    ...
    if st.button("🚀 Predict Churn", type="primary"):
        try:
            ...
            result_df["Churn Probability"] = model.predict_proba(X_arr)[:, 1].round(4)
            ...
            display_cols = ["Churn Probability", "Risk Tier"] + [
                c
            ...
            st.info(
                "💡 For SHAP explanation of any individual customer, use the **🧍 Single Customer** page."
            )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(
        "Search any customer from your uploaded dataset by their ID — instant prediction, no manual entry."
    )

    # ── Guard: need predictions to have been run first ───────────────────
    if "prediction_result" not in st.session_state:
        st.warning(
            "⚠️ Upload and predict a dataset first on the **🔮 Predict Churn** page."
        )
        st.info(
            "Once you run predictions, come back here to look up any individual customer."
        )
        st.stop()
    ...
    with btn_col:
        search_clicked = st.button(
            "🔍 Look up", type="primary", use_container_width=True
        )
    ...
    with st.expander("Or browse all customers", expanded=False):
        ...
        browsed_id = st.selectbox(
            "Select a customer", options=["— select —"] + all_ids, key="browse_id"
        )
    ...
    if customer_id and customer_id.strip():
        ...
        try:
            ...
            if mapped_base_df is not None:
                ...
                base_vals = (
                    mapped_base_df.loc[row_idx].to_dict()
                    if row_idx in mapped_base_df.index
                    else None
                )
            else:
            ...
            if base_vals:
                ...
                st.caption(
                    "Raw feature data not available for SHAP — run from the Predict page first."
                )
        ...
        if "Top churn reasons" in row.index and str(row["Top churn reasons"]) != "—":
            st.markdown(f"**Quick reasons:** `{row['Top churn reasons']}`")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    c2.metric(
        "High Risk", f"{high:,}", delta=f"{high/total:.0%}", delta_color="inverse"
    )
    c3.metric(
        "Medium Risk", f"{medium:,}", delta=f"{medium/total:.0%}", delta_color="off"
    )
    c4.metric("Low Risk", f"{low:,}", delta=f"{low/total:.0%}", delta_color="off")
    ...
    with col2:
        ...
        if "source_industry" in result_df.columns:
            industry = (
                result_df.groupby(["source_industry", "Risk Tier"])
                .size()
    ...
    try:
        ...
        st.caption(
            "💡 For per-customer SHAP explanations, use the **🧍 Single Customer** page."
        )
    except AttributeError:
    ...
    with f_col2:
        industry_opts = (
            ["All"] + sorted(result_df["source_industry"].dropna().unique().tolist())
            if "source_industry" in result_df.columns
            else ["All"]
        )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(
        "Live view of all training experiments, model versions, and production status."
    )
    ...
    if (PROJECT_ROOT / "mlruns.db").exists():
        ...
        tracking_uri = (
            f"file:///{mlruns_path}"
            if mlruns_path[1:3] == ":/" or mlruns_path[0] == "/"
            else f"file:///{mlruns_path}"
        )
    else:
        tracking_uri = None
    ...
    try:
        ...
        if not experiments:
            st.info(
                "No MLflow experiments found. Run `python -m src.unified_churn_pipeline` to generate experiment data."
            )
        else:
            # Experiment selector
            ...
            exp_id = next(
                e.experiment_id for e in experiments if e.name == selected_exp
            )
            ...
            if not runs:
                ...
                st.caption(
                    f"🏆 Best AUC: **{best_auc:.4f}** across {len(runs_df)} runs"
                )
                ...
                # Multiple runs often exist per model (e.g. re-triggered
                # retraining on the same model type). Plotting every run
                # stacks identical/near-identical bars on top of each other
                # at the same x-position. Keep only the best run (by AUC)
                # per model type so the chart shows one clean bar per model.
                best_per_model = (
                    runs_df.loc[runs_df.groupby("Model")["AUC"].idxmax()]
                    .sort_values("AUC", ascending=False)
                    .reset_index(drop=True)
                )

                if len(best_per_model) > 0:
                    metrics_to_plot = ["AUC", "F1", "Precision", "Recall"]
                    available = [
                        m for m in metrics_to_plot if best_per_model[m].sum() > 0
                    ]
                    ...
                    for i, metric in enumerate(available):
                        fig.add_trace(
                            go.Bar(
                                name=metric,
                                x=best_per_model["Model"],
                                y=best_per_model[metric],
                    ...
                    fig.update_layout(
                        barmode="group",
                        title="Best run per model (by AUC)",
                        xaxis_title="Model",
                        yaxis_title="Score",
                    ...
                    if len(runs_df) > len(best_per_model):
                        st.caption(
                            f"Showing the best run per model type "
                            f"({len(best_per_model)} model types) out of "
                            f"{len(runs_df)} total logged runs — duplicate/"
                            f"re-triggered runs on the same model are collapsed "
                            f"to their highest-AUC run."
                        )
                else:
                    st.caption("Run at least 1 experiment to see comparison chart.")
        ...
        try:
            ...
            if not registered:
                st.info(
                    "No registered models found. Run `python -m src.unified_churn_pipeline` to train and register models."
                )
            else:
                for rm in registered:
                    versions = client.get_latest_versions(rm.name)
                    with st.expander(
                        f"**{rm.name}**  —  {len(versions)} version(s)", expanded=True
                    ):
                        for v in versions:
                            stage_color = {
                                "Production": "#59a14f",
                            ...
                            col_d.metric(
                                "Created",
                                pd.Timestamp(v.creation_timestamp, unit="ms").strftime(
                                    "%Y-%m-%d"
                                ),
                            )
                            col_r.metric(
        ...
        # ── Retraining trigger (detached background process) ───────────────
        # Retraining used to run via a *blocking* subprocess.run() call inside
        # the Streamlit script thread. That caused two problems:
        #   1. A hard 5-minute timeout that silently killed the training run
        #      while showing a misleading "running in background" message.
        #   2. If the app was stopped/refreshed while training was still
        #      blocking the thread, Streamlit's teardown raced against that
        #      blocked thread -> "RuntimeError: Event loop is closed".
        # Fix: launch training as a fully detached OS process that survives
        # the Streamlit server stopping, and track its progress via a small
        # JSON status file that the UI polls (no blocking anywhere).
        import json
        import subprocess
        import sys
        import time as _time

        RETRAIN_STATUS_FILE = PROJECT_ROOT / "retrain_status.json"
        RETRAIN_LOG_FILE = PROJECT_ROOT / "retrain_log.txt"
        RETRAIN_RUNNER = PROJECT_ROOT / "_retrain_runner.py"
...
'''
        ...
        def _read_retrain_status():
            if RETRAIN_STATUS_FILE.exists():
                try:
                    return json.loads(RETRAIN_STATUS_FILE.read_text())
                except Exception:
                    return None
            return None

        def _launch_retraining():
            RETRAIN_RUNNER.write_text(RETRAIN_RUNNER_CODE)
            RETRAIN_STATUS_FILE.write_text(
                json.dumps({"status": "running", "started": _time.time()})
            )
            popen_kwargs = {
                "cwd": str(PROJECT_ROOT),
            ...
            if os.name == "nt":
                # DETACHED_PROCESS + CREATE_NO_WINDOW: survives the parent
                # (Streamlit) exiting, and doesn't pop up a console window.
                popen_kwargs["creationflags"] = (
                    subprocess.DETACHED_PROCESS | subprocess.CREATE_NO_WINDOW
                )
            else:
                # New session so it isn't killed when the parent's process
                # group receives a terminal signal (e.g. Ctrl+C / app stop).
                popen_kwargs["start_new_session"] = True
            subprocess.Popen([sys.executable, str(RETRAIN_RUNNER)], **popen_kwargs)

        st.subheader("🔄 Retraining")
        st.caption(
            "Manually trigger a retraining run. This runs your full training pipeline "
            "and registers the new model — as a background process, so it keeps running "
            "even if you navigate away, refresh, or stop the app."
        )

        status = _read_retrain_status()
        is_running = bool(status) and status.get("status") == "running"
        ...
        with r_col1:
            ...
            if status:
                if status["status"] == "running":
                    elapsed = _time.time() - status.get("started", _time.time())
                    st.info(
                        f"⏳ Retraining in progress — running for {elapsed:.0f}s..."
                    )
                elif status["status"] == "success":
                    dur = status.get("finished", 0) - status.get("started", 0)
                    st.success(
                        f"✅ Last retraining finished successfully in {dur:.0f}s."
                    )
                    if RETRAIN_LOG_FILE.exists():
                        with st.expander("Training log"):
                            st.code(RETRAIN_LOG_FILE.read_text(), language="text")
                elif status["status"] == "failed":
                    st.error(
                        f"❌ Last retraining failed "
        ...
        with r_col2:
            if st.button(
                ...
                try:
                    ...
                    st.error(f"Could not start retraining: {train_err}")
            if is_running:
                st.caption("Button disabled while a run is in progress.")
            if st.button("🔄 Refresh status", use_container_width=True):
                st.rerun()

        # Lightweight auto-poll while a run is active, without blocking
        # the training process itself (which lives in a separate PID).
        if is_running:
            _time.sleep(2)
            st.rerun()
        ...
        st.info(
            "💡 MLflow tracking database not found — showing last training results."
        )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with t3:
        ...
        <div style='background:rgba(99,102,241,0.08); border:1px solid rgba(99,102,241,0.2);
            ...
            <div style='font-size:13px; color:#94a3b8; line-height:2;'>
                Streamlit<br>Plotly<br>Hugging Face Spaces<br>Python 3.11
            </div>
        </div>""",
            unsafe_allow_html=True,
        )
    ...
    steps = [
        (
            "📥",
            "Data ingestion",
            "Raw CSV from 3 industries merged into unified schema",
        ),
        (
            "🔧",
            "Feature engineering",
            "17 cross-industry features including RFM-style scores",
        ),
        ...
        (
            "📊",
            "Experiment tracking",
            "Every run logged to MLflow with metrics + artifacts",
        ),
